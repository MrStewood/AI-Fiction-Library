...
python3 tools/studio.py pack \
  --slug <book-slug> --role scene_planner --work scene_outline
```
...
## Done gate
Write `outline/SCENE_OUTLINE.md` plus per-scene contracts under `outline/scenes/`.
No manuscript prose. Projected word count must fit length profile from `book.yaml`.
