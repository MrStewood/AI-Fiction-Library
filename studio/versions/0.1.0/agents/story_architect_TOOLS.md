...
python3 tools/studio.py pack \
  --slug <book-slug> --role story_architect --work story_bible
```
...
## Work modes
Use `--work` for the assigned stage: `creative_brief`, `premise`, `story_bible`, `beat_sheet`.
See `studio works`. Deliverable path comes from the pack job card.
